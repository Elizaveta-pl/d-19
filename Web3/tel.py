...
def clear_simv(phone_number):
    ...
    if phone_number[1:].isalnum():
        ...
        raise Exception('В номере введены недопустимые символы')
    ...
    for i in phone_number:
        if i == '(':
            ...
            if (len(stack) > 0):
                ...
                raise Exception('Номер введены не парные скобки')
    if len(stack) == 0 and bracket == 1:
        return True
    else:
        raise Exception('В номер допускается ввод одной пары скобок')
...
def tel_clear(phone_number):
    ...
    if phone_number[:1] in spis or phone_number[:2] in spis:
        ...
        raise Exception('Номер должен начинатся с 8 или +7')
